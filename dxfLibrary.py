...
class _Entity(_Call):
	...
	def __init__(self,paperspace=None,color=None,layer='0',
				 ...
				 extrusion=None,elevation=None,thickness=None,  visible=None,
				 parent=None):
...
class Drawing(_Collection):
	...
	def __str__(self):
		...
		header=[self.acadver,self.dxfcodepage]+[self._point(attr,getattr(self,attr))+'\n' for attr in _HEADER_POINTS]
	...
	def _write_section(self,file,name,data):
		...
		for x in data:
			if not isinstance(x,str):
				x = x.__str__()
			file.write(x.encode("cp932").decode("cp932")) # dxf R12 files are expected to be in that encoding
		file.write('  0\nENDSEC\n')
	...
	def save(self):
		outfile=open(self.fileName,'w',encoding='cp932', errors='replace')
