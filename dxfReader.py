...
def readDXF(filename):
	"""Reads a DXF file and returns a drawing object."""
	#【超重要】Windowsの日本語パスをバグらせないための変換をここに入れます
	safe_filename = os.path.abspath(filename)

	#上で変換した「safe_filename」を使ってファイルを開きます
	infile = open(safe_filename, 'r', encoding='cp932', errors='ignore')
	...
	try:
		...
		if drawing:
			#「self_filename」になっていたスペルミスを「safe_filename」に修正します
			drawing.name = safe_filename
